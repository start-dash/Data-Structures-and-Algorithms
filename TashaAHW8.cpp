...
std::priority_queue<int> max_heap;                                      // STL Max-heap
    ...
    for (int i = 1; i < data.size(); i++) {
        ...
        if (max_heap.size() > min_heap.size()) { // (case 1) max heap has more elements
            if (x < median) {
        ...
        else if (max_heap.size() < min_heap.size()) { // (case 2) min heap has more elements
            if (x > median) {
        ...
        else if (max_heap.size() == min_heap.size()) { // (case 3) both heaps are balanced
            if (x < median) {
                max_heap.push(x);
