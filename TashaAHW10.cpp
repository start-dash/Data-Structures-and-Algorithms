...
void DFSHelper(const adjlist& alist, vector<int>& dfslist, vector<bool>& visited, int source) {
    ...
    for (int i = 0; i< alist[source].size(); i++) {
        int visitor = alist[source][i].first;
        if (!visited[visitor]) {
            DFSHelper(alist, dfslist, visited, visitor);
        }
    }
}
...
vector<int> BFS(const adjlist& alist, int source) {
    ...
    while(!queue.empty()) {
        ...
        queue.pop();
        if (!visited[cursor]) {
            BFSlist.push_back(cursor);
            visited[cursor] = true;
            for (int i = 0; i < alist[cursor].size(); i++) {
                int visitor = alist[cursor][i].first;
                queue.push(visitor);
            }
        }
    }
